-- RPC: seed_standard_coa(p_entity_id uuid)
-- Seeds all 67 standard church COA accounts for a given entity.
-- Safe to re-run — uses ON CONFLICT (code) DO NOTHING.
-- Called automatically when a new Accounting Book is created.

CREATE OR REPLACE FUNCTION seed_standard_coa(p_entity_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, sort_order, entity_id) VALUES
    ('A',   'Assets',         'Asset',     1, false, 10, p_entity_id),
    ('L',   'Liabilities',    'Liability', 1, false, 20, p_entity_id),
    ('EQ',  'Equity & Funds', 'Equity',    1, false, 30, p_entity_id),
    ('INC', 'Income',         'Income',    1, false, 40, p_entity_id),
    ('EXP', 'Expenses',       'Expense',   1, false, 50, p_entity_id)
  ...
  SELECT v.code, v.name, 'Asset', 2, false, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
    ('A-CA',  'Current Assets',   10),
    ('A-FA',  'Fixed Assets',     20),
    ('A-INV', 'Investments',      30),
    ('A-ADV', 'Advances & Loans', 40)
  ) AS v(code, name, so) WHERE p.code = 'A' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Liabilities
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Liability', 2, false, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
    ('L-CL', 'Current Liabilities',   10),
    ('L-LL', 'Long-term Liabilities', 20)
  ) AS v(code, name, so) WHERE p.code = 'L' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Equity
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Equity', 2, false, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
    ('EQ-CF', 'Church Funds', 10),
    ('EQ-RS', 'Reserves',     20)
  ) AS v(code, name, so) WHERE p.code = 'EQ' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Income
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Income', 2, false, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
    ('INC-CH', 'Church Collections', 10),
    ('INC-OT', 'Other Income',       20)
  ) AS v(code, name, so) WHERE p.code = 'INC' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Expenses
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Expense', 2, false, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
    ('EXP-ST', 'Staff & Clergy',      10),
    ('EXP-AD', 'Administration',      20),
    ('EXP-MN', 'Ministry & Outreach', 30),
    ('EXP-MT', 'Maintenance',         40)
  ) AS v(code, name, so) WHERE p.code = 'EXP' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  -- Current Assets
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Asset', 3, true, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
  ...
  ) AS v(code, name, so) WHERE p.code = 'A-CA' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'A-FA' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'A-INV' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'A-ADV' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Current Liabilities
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Liability', 3, true, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
  ...
  ) AS v(code, name, so) WHERE p.code = 'L-CL' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'L-LL' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Church Funds
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Equity', 3, true, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
  ...
  ) AS v(code, name, so) WHERE p.code = 'EQ-CF' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'EQ-RS' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Church Collections
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Income', 3, true, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
  ...
  ) AS v(code, name, so) WHERE p.code = 'INC-CH' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'INC-OT' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;

  -- Staff & Clergy
  INSERT INTO chart_of_accounts (code, name, account_type, level, is_postable, parent_id, sort_order, entity_id)
  SELECT v.code, v.name, 'Expense', 3, true, p.id, v.so, p_entity_id
  FROM chart_of_accounts p, (VALUES
  ...
  ) AS v(code, name, so) WHERE p.code = 'EXP-ST' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'EXP-AD' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'EXP-MN' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
  ...
  ) AS v(code, name, so) WHERE p.code = 'EXP-MT' AND p.entity_id = p_entity_id
  ON CONFLICT (code) DO NOTHING;
